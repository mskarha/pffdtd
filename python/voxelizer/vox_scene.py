...
class VoxScene:
    ...
    def calc_adj(self, Nprocs=None):
        ...
        def process_voxel(idx, proc_idx):
            ...
            for tri_pre, tri_ind in zip(vox.tris_pre, vox.tri_idxs):
                ...
                bb_mask = (
                    np.all(xyz_vox >= tbmin - hf * (1 + R_EPS), axis=-1) &
                    np.all(xyz_vox <= tbmax + hf * (1 + R_EPS), axis=-1)
                ).reshape(vox_shape)
                ...
                for k in range(NN):
                    ...
                    hit_dist = np.full(vox_shape, np.inf)
                    _, hit_dist.flat[ray_mask.flat[:]] = tri_ray_intersection_vec(
                        ray_o, ray_d, npa([tri_pre]), d_eps=1.0e-3 * h
                    )
        ...
        if Nprocs == 1:  # no need to use mp
            ...
            procs = []
            idx_lists = [[] for _ in range(Nprocs)]
        ...
        for idx in range(Nvox_nonempty):
            ...
            bn_ixyz_vox = (bn_iz + iz_start) + (bn_iy + iy_start) * Nz + (bn_ix + ix_start) * Ny * Nz
    ...
    def draw(self, backend='mayavi'):
        ...
        for i in range(-1, Nmat):  # zero-indexing, -1 is rigid
            ...
            if backend == 'mayavi':
                if i == -1:
                    mlab.points3d(xv[bn_ix[qq]], yv[bn_iy[qq]], zv[bn_iz[qq]], color=color, mode='cube', scale_factor=sf)
                else:
                    mlab.points3d(xv[bn_ix[qq]], yv[bn_iy[qq]], zv[bn_iz[qq]], color=color, mode='sphere', resolution=8, scale_factor=sf)

            elif backend == 'polyscope':
                ps_cloud_in = ps.register_point_cloud(mat, np.c_[xv[bn_ix[qq]], yv[bn_iy[qq]], zv[bn_iz[qq]]], color=color)
...
def main():
    ...
    parser.add_argument('--area_eps', type=float, help='for pruning degenerate triangles')
