...
class SimEngine:
    ...
    def run_plot(self,nsteps=1,draw_backend='mayavi',json_model=None):
        self.print('running..')
        self.ct = 0  # Initialize the frame counter as an instance attribute
        Nx = self.Nx
        Ny = self.Ny
        Nz = self.Nz
        Nt = self.Nt
        ...
        print("iz max: ", np.max(iz))
        print("ix: ", ix)
        print("ix_in: ", ix_in)
        print("ix min: ", np.min(ix))
        print("ix max: ", np.max(ix))
        print("iy: ", iy)
        print("iy_in: ", iy_in)
        ...
        if draw_backend == 'matplotlib':
            ...
            #ms_counter = fig.text(0.99, 0.02, "Time: 0 ms", ha='right', va='bottom', fontsize=12)
            ...
            def update(frame):
                ...
                # Compute decibel values: 20 * log10(u / 20e-6)
                uxy_db = uxy ** 2
                uxz_db = uxz ** 2
                uyz_db = uyz ** 2

                # Replace -inf or NaN with a minimum value (e.g., -100 dB)
                uxy_db = np.nan_to_num(uxy_db, nan=-100, neginf=-100)
                uxz_db = np.nan_to_num(uxz_db, nan=-100, neginf=-100)
                uyz_db = np.nan_to_num(uyz_db, nan=-100, neginf=-100)
                ...
                self.ct += 1  # Increment the counter
                print(self.ct)
        ...
        for n in range(0,Nt,nsteps):
            ...
            if draw_backend=='matplotlib':
                ...
                hh_xz.set_clim(vmin=0,vmax=.0002)
